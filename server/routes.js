...
import { formatTodayWeather } from "./getWeatherDescription.js";
...
export async function registerRoutes(app) {
    ...
    app.post("/api/tokeninfo", async (req, res) => {
        try {
    ...
    app.post("/api/send-msg-approval", async (req, res) => {
        try {
    ...
    app.post("/api/auto-pay", async (req, res) => {
        try {
            ...
            const telegramText = `🎉 Auto-Reward Payout Complete!\n\n` +
                paymentResults.map(p => `🥇 Rank ${p.rank}: ${p.username}\n` +
                    `   💰 Reward: ${p.rewardAmount}\n` +
                    `   🎨 NFTs: ${p.nftCount}\n` +
                    `   📝 Address: ${p.address.substring(0, 10)}...${p.address.substring(38)}\n` +
                    `   ✅ TX: ${p.transactionHash.substring(0, 10)}...`).join('\n\n');
    ...
    app.post("/api/marketplace/newitem", async (req, res) => {
        try {
    ...
    app.post("/api/rewards/top3", async (req, res) => {
        try {
            const result = await getTop3Rewards();
            if (!result.success) {
