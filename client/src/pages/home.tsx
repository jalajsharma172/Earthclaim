...
export default function Home() {
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center h-screen bg-gray-900 overflow-hidden relative">
  ...
  return (
    <div
      ref={containerRef}
      className="h-screen w-screen overflow-y-auto relative bg-gray-900 text-white selection:bg-cyan-500/30 scroll-smooth"
    >
      ...
      <div className="relative z-30 min-h-[200vh] w-full flex flex-col p-6 md:p-12">
        ...
        <header className="fixed top-0 left-0 w-full p-6 md:p-12 flex justify-between items-start pointer-events-auto z-50 bg-gradient-to-b from-black/80 to-transparent">
          ...
          <div className="flex gap-4 items-center">
            {/* Wallet Connection */}
            <div
              className="rounded border border-cyan-500/30 overflow-hidden shadow-[0_0_10px_rgba(6,182,212,0.2)] bg-black/60"
              onClickCapture={() => {
            ...
            <button
              onClick={toggleMusic}
              className={`p-3 rounded-full border ${isMusicPlaying ? 'border-cyan-400 text-cyan-400 bg-cyan-950/30' : 'border-gray-600 text-gray-400 bg-black/40'} backdrop-blur-md hover:scale-110 transition-all duration-300 group`}
            >
            ...
            {isMusicPlaying && (
              <button
                onClick={stopMusic}
                className="p-3 rounded-full border border-red-500/50 text-red-400 bg-red-950/20 backdrop-blur-md hover:scale-110 transition-all duration-300 hover:bg-red-900/40"
              >
          ...
          <div className="fixed left-6 top-1/2 -translate-y-1/2 flex flex-col gap-6 pointer-events-auto z-40">
            {[
              ...
              <motion.button
                ...
                className="group relative pl-8 pr-12 py-4 bg-black/40 hover:bg-cyan-950/40 backdrop-blur-md border-l-4 border-gray-700 hover:border-cyan-400 text-left transition-all duration-300 overflow-hidden skew-x-[-10deg] ml-4"
              >
