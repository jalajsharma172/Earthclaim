...
const Create = ({ marketplace, nft, account }: CreateProps) => {
  ...
  const [_marketplace, _setMarketplace] = useState<ethers.Contract | null>(null)
  ...
  const decodeError = (errorData: string) => {
  ...
  const createNFT = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        const isAllApproved: boolean = await nftContract.isApprovedForAll(walletAddress, MARKETPLACE_ADDRESS);
        if (isAllApproved) {
      ...
      const makeItemFn = (marketplace_contract as any).makeItem;
            ...
            <div className="flex items-center justify-between">
              ...
              {!signer && !isConnecting && (
                <button
                  ...
                  className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white font-mono rounded-lg border border-blue-400 transition-all duration-300"
                >
                  Connect Wallet
                </button>
              )}
