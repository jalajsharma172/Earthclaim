...
export default function Dashboard({ account }) {
    ...
    const [mintingNFTs, setMintingNFTs] = useState(new Set());
    ...
    const mintNFTFromHash = async (nft, metadataURI) => {
        ...
        try {
            ...
            if (receipt.status === 1) {
                ...
                const updatedData = nftData?.map(item => item.IPFShashcode === nft.IPFShashcode
            ...
            if (err.code === "ACTION_REJECTED")
                setError("Mission Aborted by Agent");
            else if (err.code === "INSUFFICIENT_FUNDS")
                setError("Insufficient Energy Credits (ETH)");
            else
                setError(err.message || "Minting Protocol Failed");
        }
        finally {
    ...
    //Free NFTs
    const fetechNFTs = async (walletAddress) => {
        setLoading(true);
        try {
    ...
    return (<div className="min-h-screen relative overflow-hidden text-cyan-50 font-sans selection:bg-cyan-500/30">
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        ].map((btn, idx) => (<button key={idx} onClick={() => navigate(btn.path)} className={`group relative px-8 py-4 backdrop-blur-md bg-gradient-to-r ${btn.color} border rounded-sm transition-all duration-300 hover:scale-105 active:scale-95`}>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 max-w-7xl mx-auto">
          ...
          <div className="xl:col-span-1 space-y-6">
            ...
            <div className="bg-black/60 backdrop-blur-xl border border-cyan-500/30 p-6 rounded relative overflow-hidden group">
              ...
              <div className="space-y-4">
                ...
                {connectionStatus !== 'connected' ? (<div className="w-full py-4 text-center border border-cyan-500/30 text-cyan-500/50 font-mono text-xs uppercase">
                    Connect via Navbar
          ...
          <div className="xl:col-span-2">
            <div className="h-full bg-black/60 backdrop-blur-xl border border-cyan-500/20 rounded p-6 relative">
              ...
              {connectionStatus !== "connected" ? (<div className="h-64 flex flex-col items-center justify-center border border-dashed border-cyan-900 rounded bg-cyan-950/10">
                  <Wifi className="w-12 h-12 text-cyan-800 mb-4"/>
                  <p className="font-mono text-cyan-700">SECURE LINK REQUIRED FOR DATA STREAM</p>
                </div>) : (<div className="space-y-4 max-h-[600px] overflow-y-auto custom-scrollbar pr-2">
                  {loading && (<div className="text-center py-12 text-cyan-500 font-mono animate-pulse">Requesting Satellite Data...</div>)}

                  {!loading && nftData?.map((item, idx) => {
                const isProcess = isMinting(item.IPFShashcode);
                const isMinted = item.minted === true;
                return (<div key={idx} className={`relative p-4 border ${isMinted ? 'border-green-900/50 bg-green-950/10' : 'border-cyan-500/20 bg-cyan-950/10'} hover:border-cyan-400/50 transition-all roundedgroup`}>
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                          ...
                          <div className="flex items-center gap-2">
                            <button onClick={() => navigate('/view-polygon', {
                        state: {
                            ...
                            {!isMinted ? (<button onClick={() => handleMintClick(item)} disabled={isProcess} className={`px-6 py-2 rounded text-xs font-mono font-bold uppercase tracking-wider transition-all shadow-[0_0_10px_rgba(0,0,0,0.5)] ${isProcess
                            ...
                            : 'bg-green-600 hover:bg-green-500 text-white border border-green-500'}`}>
                              ...
                              </button>) : (<button onClick={() => navigate(`/token-info/${item.IPFShashcode}`, { state: { ...item, signer } })} className="px-6 py-2 bg-blue-600/20 hover:bg-blue-500/30 text-blue-300 border border-blue-500/50 rounded text-xs font-mono uppercase tracking-wider">
                                Asset Data
                              </button>)}
                          </div>
                        </div>
        ...
        {/* Transaction Overlay */}
        {transactionHash && (<div className="fixed bottom-8 right-8 max-w-sm bg-black/90 border border-green-500 text-green-400 p-4 rounded shadow-[0_0_20px_rgba(34,197,94,0.3)] z-50 font-mono text-xs animate-slide-up">
        ...
        {showMintModal && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            ...
            <div className="mb-6">
                  ...
                  <input type="file" accept="image/*" onChange={(e) => setNftImage(e.target.files ? e.target.files[0] : null)} className="w-full text-xs text-cyan-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-cyan-900/30 file:text-cyan-400 hover:file:bg-cyan-900/50 cursor-pointer border border-cyan-900 rounded p-2 bg-black/50"/>

                  {nftImage && (<div className="mt-4 p-2 border border-cyan-500/30 rounded bg-black/40 text-center">
                      <p className="text-[10px] text-cyan-400 mb-2 font-mono uppercase tracking-widest">Preview</p>
                      <img src={URL.createObjectURL(nftImage)} alt="Survey Preview" className="max-h-48 mx-auto rounded border border-cyan-900/50 shadow-[0_0_15px_rgba(6,182,212,0.1)]"/>
                    </div>)}
                </div>) : (
            ...
            <div className="mb-6">
                  <div className="space-y-4 mb-4">
                    <div>
                      ...
                      <input type="text" value={nftName} onChange={(e) => setNftName(e.target.value)} className="w-full bg-black/40 border border-cyan-500/30 rounded p-2 text-sm text-white focus:border-cyan-400 outline-none font-mono" placeholder="Enter token name"/>
                    </div>
                    <div>
                      <label className="block text-xs font-mono text-cyan-300 mb-1">DESCRIPTION</label>
                      <textarea value={nftDescription} onChange={(e) => setNftDescription(e.target.value)} className="w-full bg-black/40 border border-cyan-500/30 rounded p-2 text-sm text-white focus:border-cyan-400 outline-none font-mono h-24 resize-none" placeholder="Enter description"/>
                    </div>
                  </div>
              ...
              <div className="flex gap-4 justify-end">
                <button onClick={handleModalClose} className="px-4 py-2 rounded text-xs font-mono font-bold uppercase tracking-wider bg-slate-700 hover:bg-slate-600 text-slate-300 border border-slate-600 transition-all" disabled={isUploading}>
                  Cancel
                </button>

                {modalStep === 1 ? (<button onClick={handleNextStep} disabled={!nftImage || isUploading} className={`px-4 py-2 rounded text-xs font-mono font-bold uppercase tracking-wider transition-all shadow-[0_0_15px_rgba(34,197,94,0.4)] ${!nftImage || isUploading
                    ? 'bg-gray-600 text-gray-400 cursor-not-allowed border border-gray-500'
                    : 'bg-blue-600 hover:bg-blue-500 text-white border border-blue-500'}`}>
                    {isUploading ? 'UPLOADING...' : 'NEXT ->'}
                  </button>) : (<button onClick={handleModalSubmit} className="px-4 py-2 rounded text-xs font-mono font-bold uppercase tracking-wider bg-green-600 hover:bg-green-500 text-white border border-green-500 transition-all shadow-[0_0_15px_rgba(34,197,94,0.4)]">
                    Create NFT
                  </button>)}
              </div>
            </div>
