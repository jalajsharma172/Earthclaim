...
const Navbar = ({ setNft, setMarketplace, setAccount, setUser }) => {
    ...
    const NavItem = ({ href, label, icon }) => {
        ...
        <a className={`group flex items-center gap-2 px-4 py-2 relative transition-all duration-300 ${isActive
          ...
          {isActive && (<div className="absolute top-0 left-0 w-full h-[2px] bg-cyan-400 shadow-[0_0_10px_rgba(6,182,212,0.8)]"></div>)}
    ...
    return (<nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 py-4 pointer-events-none bg-gradient-to-b from-black/90 via-black/40 to-transparent backdrop-blur-sm border-b border-white/5">
      ...
      <div className="flex items-center gap-4 pointer-events-auto">
        ...
        {!activeAccount ? (<div className="rounded border border-cyan-500/30 overflow-hidden shadow-[0_0_10px_rgba(6,182,212,0.2)] bg-black/60">
            ...
            <button onClick={() => setShowDropdown(!showDropdown)} className="flex items-center gap-3 px-4 py-2 bg-black/60 hover:bg-cyan-950/40 backdrop-blur-md border border-cyan-500/30 hover:border-cyan-400/60 transition-all rounded group">
              {/* Avatar */}
              <div className="relative">
                ...
                <img src={avatarUrl} alt="Avatar" className="relative w-6 h-6 rounded-full border border-cyan-500/50"/>
            ...
            {/* Dropdown Menu */}
            {showDropdown && (<div className="absolute right-0 mt-2 w-56 bg-black/95 backdrop-blur-xl border border-cyan-500/20 shadow-2xl rounded overflow-hidden z-50">
                ...
                <div className="p-1">
                  <button onClick={handleDisconnect} className="w-full flex items-center gap-3 px-3 py-2 hover:bg-red-950/20 transition-colors font-mono text-xs tracking-wide text-left text-gray-400 hover:text-red-400 rounded-sm group">
        ...
        <button onClick={() => {
            handleLogout();
            navigate('/');
        }} className="w-8 h-8 flex items-center justify-center rounded border border-gray-800 hover:border-red-500/30 hover:bg-red-950/10 text-gray-500 hover:text-red-400 transition-colors" title="System Logout">
          <LogOut size={14}/>
        </button>
      </div>
    </nav>);
