...
import { getUserPathByUsername } from './map-helpers';
...
function MapView() {
    ...
    const [userPath, setUserPath] = useState([]); // Save it in USERDATA PATH
    ...
    const [freePolygons, setFreePolygons] = useState([]);
    const [currentFreePolygonIndex, setCurrentFreePolygonIndex] = useState(0);
    const freePolygonLayerRef = useRef(null);
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            ...
            }, err => console.error("Error getting location:", err), { enableHighAccuracy: true, timeout: 5000, maximumAge: 0 });
        }, timerInterval);
    ...
    //Fetch UserPath
    const getUserPathDirectly = async (username) => {
        try {
            if (!username) {
            ...
            if (Array.isArray(result.data)) {
                ...
                const validPath = result.data.filter(point => point && typeof point.lat === 'number' && typeof point.lon === 'number');
    ...
    useEffect(() => {
        ...
        const latlngs = userPath.map(p => [p.lat, p.lon]);
    ...
    const createPolygonFromPath = useCallback(() => {
        ...
        const closureIndex = userPath.findIndex((point, index) => index < userPath.length - 1 && // Don't match the last point itself
    ...
    useEffect(() => {
        ...
        if (!currentPoly || !currentPoly.coordinates)
            return;
        ...
        const latlngs = currentPoly.coordinates.map((coord) => [coord[1], coord[0]]);
    ...
    return (<div style={{ position: 'relative', height: '100vh', width: '100%', background: '#0f172a' }}>
      ...
      {!gameActive && (<div style={{ position: 'absolute', top: '20px', right: '20px', zIndex: 1000 }}>
          <button onClick={() => navigate("/dashboard")} style={homeButtonStyle}>
      ...
      {gameActive && !showHud && (<div style={{ position: 'absolute', top: '20px', right: '20px', zIndex: 1000 }}>
      ...
      {gameActive && showHud && (<div style={controlPanelStyle}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
            ...
            <button onClick={() => setShowHud(false)} style={{
          ...
          <div style={{ display: 'flex', gap: '8px', flexDirection: 'column' }}>
            ...
            <button onClick={() => saveUserPathAsync()} style={{
                ...actionButtonStyle,
                background: 'linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)',
            ...
            {isLoopClosed && (<button onClick={() => setShowNameForm(true)} style={{
                    ...actionButtonStyle,
                    background: 'linear-gradient(135deg, #00ff88 0%, #00cc6a 100%)',
            ...
            {isLoopClosed && (<button onClick={savePolygonFromPath} style={{
                    ...actionButtonStyle,
            ...
            {showNameForm && (<div style={{
                ...
                <input value={polygonName} onChange={(e) => setPolygonName(e.target.value)} placeholder="Enter name..." style={{
                    width: '100%',
                    padding: '8px',
                    borderRadius: 4,
      ...
      {!freeMode && (<div style={statusStyle}>
          <div style={{
                width: '8px',
                height: '8px',
                borderRadius: '50%',
      ...
      {showMap && freeMode && (<>
          <button onClick={() => setCurrentFreePolygonIndex(prev => (prev - 1 + freePolygons.length) % freePolygons.length)} style={{
                position: 'absolute',
                bottom: '20px',
                left: '20px',
                zIndex: 1000,
          ...
          <button onClick={() => setCurrentFreePolygonIndex(prev => (prev + 1) % freePolygons.length)} style={{
                position: 'absolute',
                bottom: '20px',
                right: '20px',
                zIndex: 1000,
