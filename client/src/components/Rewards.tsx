...
interface Offer {
  ...
  description: string;
  pointsValue: number;
  category: 'survey' | 'app' | 'financial' | 'gaming' | 'shopping' | 'crypto';
  icon: string;
  color: string;
  requirements: number; // Number of NFTs required
...
// Interface for UserNFT data (same as Dashboard)
interface UserNFTData {
  id: string;
  username: string;
  hashjson: string;
  minted: number; // 0 = false (not minted), 1 = true (minted)
  usedForReward?: number; // 0 = false, 1 = true (used for rewards)
  createdAt: string;
}
...
export default function Rewards() {
  ...
  const canClaimOffer = (offer: Offer): boolean => {
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-blue-600">
                  {formatPoints(userRewards.reduce((sum: number, reward: UserReward) => sum + reward.pointsEarned, 0))}
                </div>
